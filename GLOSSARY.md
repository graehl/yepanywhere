...
| `compose-time-context-anchors` | Staleness anchors (`(Ns ago)` / `(Ms later)`) prepended to queued/deferred user turns at delivery time so the agent knows how old a queued message is relative to work just done. | [compose-time-context-anchors](topics/compose-time-context-anchors.md) |
