...
| `compose-time-context-anchors` | Opt-in (default off): queued/deferred user turns are delivered verbatim, one per delivery boundary; `YA_COMPOSE_ANCHORS=1` prepends `(Ns ago)` / `(Ms later)` staleness anchors and `YA_DEFERRED_BATCH_FLUSH=1` flushes all eligible turns as one separator-joined provider turn. | [compose-time-context-anchors](topics/compose-time-context-anchors.md) |
